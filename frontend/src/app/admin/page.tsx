    ...
    <div className="min-h-screen bg-neutral-950 text-neutral-50 flex">
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 border-r border-white/5 bg-neutral-900/95 backdrop-blur-xl flex flex-col transform transition-transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0`}>
        ...
        <nav className="flex-1 px-4 py-6 space-y-2">
          <Link href="/admin" className="flex items-center gap-3 px-4 py-3 bg-teal-600/10 text-teal-300 rounded-xl font-medium">
      ...
      <main className="flex-1 flex flex-col max-h-screen overflow-y-auto">
        <header className="flex items-center justify-between px-6 py-4 border-b border-white/5 bg-neutral-950/80 sticky top-0 z-30 backdrop-blur-md">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-xl font-bold flex items-center gap-2 hidden md:flex"><Scale className="w-6 h-6 text-teal-400"/> Venture Studio Console</h1>
        ...
        {loading || !stats ? (
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
              <div className="bg-gradient-to-br from-teal-900/40 to-neutral-900/50 border border-teal-500/20 rounded-2xl p-6 relative overflow-hidden">
                <Users className="absolute -right-4 -bottom-4 w-24 h-24 text-teal-500/10" />
                <h3 className="text-sm font-medium text-teal-300 mb-1">Total Citizens</h3>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="lg:col-span-2 space-y-8">
                ...
                <div className="bg-neutral-900/50 border border-white/10 rounded-2xl p-8">
                  <div className="flex justify-between items-center mb-6">
                    <h3 className="text-xl font-bold flex items-center gap-2"><Plus className="w-5 h-5 text-teal-400" /> Provision New Firm</h3>
                  </div>
                  ...
                  <form onSubmit={handleCreateFirm} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input type="text" required value={firmName} onChange={e=>setFirmName(e.target.value)} className="w-full bg-neutral-800 border border-white/10 rounded-lg py-2 pl-9 pr-3 text-sm focus:outline-none focus:border-teal-500" placeholder="e.g. Khaitan & Co" />
                      </div>
                    </div>
                    <div>
                      ...
                      <div className="flex gap-4 items-center">
                        <div className="relative flex-1">
                          ...
                          <input type="number" required={!isUnlimited} disabled={isUnlimited} value={isUnlimited ? '' : quota} onChange={e=>setQuota(parseInt(e.target.value))} className="w-full bg-neutral-800 border border-white/10 rounded-lg py-2 pl-9 pr-3 text-sm focus:outline-none focus:border-teal-500 disabled:opacity-50" placeholder={isUnlimited ? "∞" : "500"} />
                        </div>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input type="email" required value={email} onChange={e=>setEmail(e.target.value)} className="w-full bg-neutral-800 border border-white/10 rounded-lg py-2 pl-9 pr-3 text-sm focus:outline-none focus:border-teal-500" placeholder="admin@khaitan.com" />
                      </div>
                    </div>
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input type="password" required value={password} onChange={e=>setPassword(e.target.value)} className="w-full bg-neutral-800 border border-white/10 rounded-lg py-2 pl-9 pr-3 text-sm focus:outline-none focus:border-teal-500" placeholder="••••••••" />
                      </div>
                    </div>
                    <div className="md:col-span-2 mt-2">
                      <Button type="submit" disabled={creating} className="w-full bg-teal-600 hover:bg-teal-500 text-white">
                ...
                <div className="bg-neutral-900/50 border border-white/10 rounded-2xl overflow-hidden">
                  <div className="p-6 border-b border-white/5">
                     <h3 className="text-xl font-bold flex items-center gap-2"><Database className="w-5 h-5 text-teal-400" /> Registered Organizations</h3>
                  </div>
              ...
              <div className="bg-neutral-900/50 border border-white/10 rounded-2xl p-6 flex flex-col h-fit sticky top-24">
                <h3 className="text-lg font-bold flex items-center gap-2 mb-6"><Activity className="w-5 h-5 text-teal-400" /> Recent Network Activity</h3>
                  ...
                  {stats.recent_users.map((u: any, i: number) => (
                    <div key={i} className="flex gap-4 items-start p-3 rounded-xl hover:bg-white/5 transition-colors">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${
                        u.role === 'citizen' ? 'bg-teal-500/20 text-teal-400' :
                        u.role === 'lawyer' ? 'bg-purple-500/20 text-purple-400' :
